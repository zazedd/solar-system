...
// #define FLOATING_POINT_DEBUG
    ...
    !defined(WITH_WASAPI) && !defined(WITH_OSS) &&                             \
    !defined(WITH_SDL1_STATIC) && !defined(WITH_SDL2_STATIC) &&                \
    !defined(WITH_ALSA) && !defined(WITH_OPENSLES) && !defined(WITH_NULL) &&   \
    !defined(WITH_COREAUDIO) && !defined(WITH_VITA_HOMEBREW) &&                \
...
void AlignedFloatBuffer::clear() { memset(mData, 0, sizeof(float) * mFloats); }

AlignedFloatBuffer::~AlignedFloatBuffer() { delete[] mBasePtr; }
...
result Soloud::init(unsigned int aFlags, unsigned int aBackend,
                    unsigned int aSamplerate, unsigned int aBufferSize,
                    unsigned int aChannels) {
  if (aBackend >= BACKEND_MAX || aChannels == 3 || aChannels == 5 ||
      aChannels == 7 || aChannels > MAX_CHANNELS)
  ...
  if (!inited &&
      (aBackend == Soloud::VITA_HOMEBREW || aBackend == Soloud::AUTO)) {
    int ret =
...
void Soloud::postinit_internal(unsigned int aSamplerate,
                               unsigned int aBufferSize, unsigned int aFlags,
                               unsigned int aChannels) {
  mGlobalVolume = 1;
  mChannels = aChannels;
  ...
  case 6:
    ...
    m3dSpeakerPosition[2 * 3 + 1] = 0;
    m3dSpeakerPosition[2 * 3 + 2] = 1;
    // Sub should be "mix of everything". We'll handle it as a special case and
    // make it a null vector.
...
void Soloud::clip_internal(AlignedFloatBuffer &aBuffer,
                           ...
                           unsigned int aSamples, float aVolume0,
                           float aVolume1) {
...
void resample(float *aSrc, float *aSrc1, float *aDst, int aSrcOffset,
              int aDstSampleCount, float /*aSrcSamplerate*/,
              float /*aDstSamplerate*/, int aStepFixed) {
#if 0
...
void panAndExpand(AudioSourceInstance *aVoice, float *aBuffer,
                  unsigned int aSamplesToRead, unsigned int aBufferSize,
                  float *aScratch, unsigned int aChannels) {
  ...
  for (k = 0; k < aChannels; k++) {
    ...
    pani[k] = (pand[k] - pan[k]) /
              aSamplesToRead; // TODO: this is a bit inconsistent.. but it's a
                              // hack to begin with
  }
...
void Soloud::mixBus_internal(float *aBuffer, unsigned int aSamplesToRead,
  ...
  for (i = 0; i < mActiveVoiceCount; i++) {
    ...
    if (voice && voice->mBusHandle == aBus &&
      ...
      while (step_fixed != 0 && outofs < aSamplesToRead) {
        if (voice->mLeftoverSamples == 0) {
          ...
          if (!voice->hasEnded() ||
            ...
            readcount = voice->getAudio(voice->mResampleData[0]->mData,
            ...
            if (readcount < SAMPLE_GRANULARITY) {
              if (voice->mFlags & AudioSourceInstance::LOOPING) {
                while (readcount < SAMPLE_GRANULARITY &&
                       voice->seek(voice->mLoopPoint, mScratch.mData,
                                   mScratchSize) == SO_NO_ERROR) {
                  voice->mLoopCount++;
                  int inc = voice->getAudio(
                      voice->mResampleData[0]->mData + readcount,
                      SAMPLE_GRANULARITY - readcount, SAMPLE_GRANULARITY);
                  readcount += inc;
                  if (inc == 0)
                    break;
                }
          ...
          // Clear remaining of the resample data if the full scratch wasn't
          // used
          if (readcount < SAMPLE_GRANULARITY) {
            unsigned int k;
            for (k = 0; k < voice->mChannels; k++)
              memset(voice->mResampleData[0]->mData + readcount +
                         SAMPLE_GRANULARITY * k,
                     0, sizeof(float) * (SAMPLE_GRANULARITY - readcount));
          }
          ...
          for (j = 0; j < FILTERS_PER_STREAM; j++) {
            if (voice->mFilter[j]) {
              voice->mFilter[j]->filter(voice->mResampleData[0]->mData,
                                        SAMPLE_GRANULARITY, voice->mChannels,
                                        voice->mSamplerate, mStreamTime);
            }
          }
        } else {
        ...
        if (voice->mSrcOffset < SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) {
          writesamples =
              ((SAMPLE_GRANULARITY * FIXPOINT_FRAC_MUL) - voice->mSrcOffset) /
                  step_fixed +
              1;

          // avoid reading past the current buffer..
          if (((writesamples * step_fixed + voice->mSrcOffset) >>
               FIXPOINT_FRAC_BITS) >= SAMPLE_GRANULARITY)
            writesamples--;
        }
        ...
        if (writesamples) {
          for (j = 0; j < voice->mChannels; j++) {
            resample(voice->mResampleData[0]->mData + SAMPLE_GRANULARITY * j,
                     ...
                     aScratch + aBufferSize * j + outofs, voice->mSrcOffset,
                     writesamples, voice->mSamplerate, aSamplerate, step_fixed);
          }
        }
      ...
      panAndExpand(voice, aBuffer, aSamplesToRead, aBufferSize, aScratch,
                   aChannels);
      ...
      // Inaudible but needs ticking. Do minimal work (keep counters up to date
      // and ask audiosource for data)
      float step = voice->mSamplerate / aSamplerate;
...
void Soloud::mapResampleBuffers_internal() {
  ...
  for (i = 0; i < mMaxActiveVoices; i++) {
    for (j = 0; j < mMaxActiveVoices; j++) {
      if (mResampleDataOwner[i] &&
          mResampleDataOwner[i] == mVoice[mActiveVoice[j]]) {
  ...
  for (i = 0; i < mMaxActiveVoices; i++) {
    if (!(live[i] & 1) &&
        mResampleDataOwner[i]) // For all dead channels with owners..
    {
  ...
  for (i = 0; i < mActiveVoiceCount; i++) {
    if (!(live[i] & 2) &&
        mVoice[mActiveVoice[i]]) // For all live voices with no channel..
    {
      int found = -1;
      ...
      mResampleDataOwner[found]->mResampleData[0] =
          &mResampleData[found * 2 + 0];
      mResampleDataOwner[found]->mResampleData[1] =
          &mResampleData[found * 2 + 1];
...
void Soloud::calcActiveVoices_internal() {
  // TODO: consider whether we need to re-evaluate the active voices all the
  // time. It is a must when new voices are started, but otherwise we could get
  // away with postponing it sometimes..
  ...
  // If we get this far, there's nothing to it: we'll have to sort the voices to
  // find the most audible.
...
void Soloud::mix_internal(unsigned int aSamples) {
  ...
  for (i = 0; i < (signed)mHighestVoice; i++) {
    if (mVoice[i] && !(mVoice[i]->mFlags & AudioSourceInstance::PAUSED)) {
      ...
      // TODO: this is actually unstable, because mStreamTime depends on the
      // relative play speed.
      if (mVoice[i]->mRelativePlaySpeedFader.mActive > 0) {
        float speed =
            mVoice[i]->mRelativePlaySpeedFader.get(mVoice[i]->mStreamTime);
      ...
      if (mVoice[i]->mVolumeFader.mActive > 0) {
        mVoice[i]->mSetVolume =
            mVoice[i]->mVolumeFader.get(mVoice[i]->mStreamTime);
        mVoice[i]->mActiveFader = 1;
  ...
  mixBus_internal(mOutputScratch.mData, aSamples, aSamples, mScratch.mData, 0,
                  (float)mSamplerate, mChannels);

  for (i = 0; i < FILTERS_PER_STREAM; i++) {
    if (mFilterInstance[i]) {
      mFilterInstance[i]->filter(mOutputScratch.mData, aSamples, mChannels,
                                 (float)mSamplerate, mStreamTime);
    }
  }

  unlockAudioMutex_internal();

  clip_internal(mOutputScratch, mScratch, aSamples, globalVolume[0],
                globalVolume[1]);
...
void deinterlace_samples_float(const float *aSourceBuffer, float *aDestBuffer,
...
void interlace_samples_float(const float *aSourceBuffer, float *aDestBuffer,
                             unsigned int aSamples, unsigned int aChannels) {
  // 111222 -> 121212
  unsigned int i, j, c;
  c = 0;
...
void interlace_samples_s16(const float *aSourceBuffer, short *aDestBuffer,
                           unsigned int aSamples, unsigned int aChannels) {
#if 0 // defined(SOLOUD_SSE_INTRINSICS) && defined(_M_IX86)
		switch (aChannels)
		{
